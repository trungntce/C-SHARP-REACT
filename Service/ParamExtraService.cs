...
public class ParamExtraService : MinimalApiService, IMinimalApi, Map.IMap
{
	public ParamExtraService(ILogger<ParamExtraService> logger) : base(logger)
	{
	}
	...
	public static IEnumerable<IDictionary> List(string? eqpCode, string? groupCode)
	{
        ...
        DataTable dt = DataContext.StringDataSet("@ParamExtra.List", RefineExpando(obj, true)).Tables[0];
	...
	public static ParamEntity? Select(string eqpCode, string paramId)
	{
		...
		return DataContext.StringEntity<ParamEntity>("@ParamExtra.Select", RefineExpando(obj, true));
	}
	...
	public static int Insert([FromBody] ParamEntity entity)
	{
		...
		return DataContext.StringNonQuery("@ParamExtra.Insert", RefineEntity(entity));
	}
	...
	public static int Update([FromBody] ParamEntity entity)
	{
		...
		return DataContext.StringNonQuery("@ParamExtra.Update", RefineEntity(entity));
	}
	...
	public static int Delete(string eqpCode, string paramId)
	{
		...
		return DataContext.StringNonQuery("@ParamExtra.Delete", RefineExpando(obj));
	}
	...
	public static int InsertSet([FromBody] ParamSetEntity entity)
	{
		...
		foreach (var i in 0..(entity.EqpCodeList.Count - 1))
		{
			...
			result += DataContext.StringNonQuery("@ParamExtra.Insert", RefineEntity(entity));
		}
	...
	public static int UpdateSet([FromBody] ParamSetEntity entity)
	{
		...
		foreach (var i in 0..(entity.EqpCodeList.Count - 1))
		{
			...
			result += DataContext.StringNonQuery("@ParamExtra.Update", RefineEntity(entity));
		}
    ...
    {
        return DataContext.StringNonQuery("@ParamExtra.DeleteModelMap", RefineEntity(entity));
    }

    [ManualMap]
    public static int InsertModelMap([FromBody] RecipeParamMapEntity entity)
    {
        return DataContext.StringNonQuery("@ParamExtra.InsertModelMap", RefineEntity(entity));
    }

    [ManualMap]
    public static int DeleteModelExtraMap([FromBody] RecipeParamMapEntity entity)
    {
        return DataContext.StringNonQuery("@ParamExtra.DeleteModelExtraMap", RefineEntity(entity));
    }

    [ManualMap]
    public static int InsertModelExtraMap([FromBody] RecipeParamMapEntity entity)
    {
        return DataContext.StringNonQuery("@ParamExtra.InsertModelExtraMap", RefineEntity(entity));
    }
	...
	public static RecipeEntity? SelectParamMap(string modelCode, string? paramId, int? operationSepNo, string? eqpCode)
	{
		...
		return DataContext.StringEntity<RecipeEntity>("@ParamExtra.SelectModelMap", RefineExpando(obj, true));
	}
