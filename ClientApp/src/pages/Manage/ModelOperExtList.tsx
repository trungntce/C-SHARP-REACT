...
import { eqpColumnDefs, modelColumnDefs, operColumnDefs } from "./ModelOperExtDefs";
...
const ModelOperExtList = (props: any) => {
  ...
  const { refetch } = useApi("modeloperext/erpmodel", getSearch, modelGridRef); 
  const { refetch: operRefetch, post, del } = useApi("modeloperext", () => {
    return { modelCode: getSelectedModel().modelCode };
  }, operGridRef); 
  ...
  const operSearchHandler = async (_?: Dictionary) => {
    ...
    const approvedoperList = await api<any>("get","modeloperext/approvedoper",{
...
export default ModelOperExtList;
