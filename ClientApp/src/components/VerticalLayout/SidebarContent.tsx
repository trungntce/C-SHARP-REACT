  ...
  return (
    <React.Fragment>      
      <SimpleBar ref={ref} className="sidebar-menu-scroll" id="nav-scroll">
        ...
        <div id="sidebar-menu">
          <ReactSortable tag="ul" className="metismenu list-unstyled" id="side-menu-favorite" list={menuList} setList={(newState: MenuItemsProps[]) => {
            sortableRef.current = newState;
          }}
          onSort={(e:Sortable.SortableEvent) => {
            setMenuList(sortableRef.current);
            
            const favMenuList = sortableRef.current.filter(x => x.className == "menu-favorite");
