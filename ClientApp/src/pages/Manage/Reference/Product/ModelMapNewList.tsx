import {
    CellClickedEvent,
    RowClassParams,
    RowDataUpdatedEvent,
    RowSelectedEvent,
  } from "ag-grid-community";
  import { useTranslation } from "react-i18next";
  import { Button, Col, Input, Row, Table } from "reactstrap";
  import { useApi, useGridRef, useSearchRef } from "../../../../common/hooks";
  import { Dictionary } from "../../../../common/types";
  import GridBase from "../../../../components/Common/Base/GridBase";
  import ListBase from "../../../../components/Common/Base/ListBase";
  import SearchBase from "../../../../components/Common/Base/SearchBase";
  import {
    columnCategoryDefs,
    columnOperEqpDefs,
    columnModelListDefs,
    columnParamDefs,
    columnRecipeDefs,
    columnDetailDefs,
  } from "./ModelMapNewDefs";
  import AutoCombo from "../../../../components/Common/AutoCombo";
  import api from "../../../../common/api";
  import { useEffect, useRef, useState } from "react";
  import { useEditRef} from "../../../../common/hooks";
  import { alertBox } from "../../../../components/MessageBox/Alert";
  import { isNullOrWhitespace } from "../../../../common/utility";
  import Select from "../../../../components/Common/Select";
  import ModelMapEdit from "./ModelMapNewEdit";
  import { showProgress } from "../../../../components/MessageBox/Progress";
  
  const ModelMapNewList = () => {
    const { t } = useTranslation();
  
    const [searchRef, getSearch] = useSearchRef();
    const [modelRef, setModelList] = useGridRef();
    const [eqpRef, setEqpList] = useGridRef();
    const [groupRef, setGroupList] = useGridRef();
    const [registedRef, setRegistedList] = useGridRef();
    const [categoryRef, setCategoryList] = useGridRef();
    const [paramRef, setParamList] = useGridRef();
    const { refetch, post } = useApi("modelrecipeparammapnew", getSearch); 
    const [editRef, setForm, closeModal] = useEditRef();
    const addRef = useRef<any>();
    const selectRef = useRef<any>();
  
    useEffect(() => {
      searchHandler();
      addRef.current.disabled = true;
    }, []);
  
    const getSelectedModel = () => {
      const rows = modelRef.current!.api.getSelectedRows();
      return rows[0];
    };
  
    const getSelectedOperEqp = () => {
      const rows = eqpRef.current!.api.getSelectedRows();
      return rows[0];
    };
  
    const getSelectedGroup = () => {
      const rows = groupRef.current!.api.getSelectedRows();
      return rows[0];
    };
  
    const searchHandler = async (_?: Dictionary) => {
      const result = await refetch();
      if (result.data) setModelList(result.data);
      setEqpList([]);
      setGroupList([]);
      setCategoryList([]);
      setParamList([]);
      setRegistedList([])
  
      addRef.current.style.backgroundColor = "#f34e4e";
      addRef.current.style.borderColor = "#f34e4e";
      const btnReg = document.getElementById('buttonRegistration');
      if (btnReg && btnReg !== undefined) { 
        btnReg.innerText = 'Recipe 승인요청';
      }
    };
  
    const modelSelectedHandler = (e: RowSelectedEvent) => {
      if (!e.node.isSelected()) return;
  
      operEqpSearchHandler(e.data.modelCode);
    };
  
    const operEqpSearchHandler = async (modelCode: string) => {
      const params = getSearch();
  
      api<any>("get", "modelrecipeparammapnew/opereqp", { approveYn: params.approveYn, modelCode : modelCode }).then(
        (result) => {
          if (result.data) {
            setEqpList(result.data[0]);
            setRegistedList(result.data[1])
          }
          setGroupList([]);
          setCategoryList([]);
          setParamList([]);
        }
      );
  
      if (params.approveYn && params.approveYn == 'S') {
        const check = await api<any>("get", "modelrecipeparammapnew/check", {modelCode: modelCode});
  
        if (check.data > 0) {
          addRef.current.style.backgroundColor = "#f34e4e";
          addRef.current.style.borderColor = "#f34e4e";
          addRef.current.disabled = false;
          const btnReg = document.getElementById('buttonRegistration');
          if (btnReg && btnReg !== undefined) { 
            btnReg.innerText = 'Recipe 승인 요청';
          }
        }
      }
      else if (params.approveYn && params.approveYn == 'N') {
        addRef.current.style.backgroundColor = "#51d28c";
        addRef.current.style.borderColor = "#51d28c";
        addRef.current.disabled = true;
        const btnReg = document.getElementById('buttonRegistration');
        if (btnReg && btnReg !== undefined) { 
          btnReg.innerText = ' 승인요청중 ';
        }
      }
    };
  
    const operEqpSelectedHandler = (e: RowSelectedEvent) => {
      if (!e.node.isSelected()) return;
  
      const params = getSearch();
      api<any>("get", "modelrecipeparammapnew/recipemap", {
        approveYn: params.approveYn, 
        equipmentCode : e.data.equipmentCode, 
        groupCode : e.data.groupCode, 
        bomItemCode : e.data.bomItemCode, 
        operationSeqNo : e.data.operationSeqNo
      }).then((result) => {
        if (result.data) setGroupList(result.data);
        setCategoryList([]);
        setParamList([]);
        result.data.map((row: any, index: string) => {
          if (row.exist == e.data.bomItemCode)
            groupRef.current?.api.getRowNode(index)?.setSelected(true);
        });
      });
    };
  
    const recipeRowSelectedHandler = (e: RowSelectedEvent) => {
      if (!e.node.isSelected()) return;
  
      api<any>("get", "recipe", {
        eqpCode: getSelectedOperEqp().equipmentCode,
        groupCode: e.data.groupCode,
      }).then((result) => {
        if (result.data) setCategoryList(result.data.filter((f:any) => f.judgeYn === "Y"));
      });
  
      api<any>("get", "param", {
        eqpCode: getSelectedOperEqp().equipmentCode,
        groupCode: e.data.groupCode,
      }).then((result) => {
        if (result.data) setParamList(result.data.filter((f:any) => f.judgeYn === "Y"));
      });
    };
  
    const regisHandler = async () => {
      const model = getSelectedModel();
      if (!model) {
        alertBox(t("@MSG_NO_MODEL_SELECTED"));
        return;
      }

      const checkReq = await api<any>("get", "modelapprove/approvecheck", {modelCode: model.modelCode});
  
      if(checkReq.data > 0){
        alertBox(t('@NG_APPROVAL_ERR'));
        operEqpSearchHandler(model.modelCode);
        return;
      }

      const check = await api<any>("get", "modelrecipeparammapnew/checkreg", {modelCode: model.modelCode});
  
      if(check.data > 0){
        alertBox("등록 절차가 완료되었습니다");
        operEqpSearchHandler(model.modelCode);
        return;
      }
  
      const data: Dictionary = {
        modelCode: model.modelCode,
      };
  
      const result = await api<any>("post", "modelrecipeparammapnew/registration", data);
      
      if (result.data > 0) {
        alertBox(t("@MSG_REGISTRATION_IS_COMPLETE"));
        operEqpSearchHandler(data.modelCode);
      }
    };
  
    const cancelHandler = async () => {
      const model = getSelectedModel();
      if (!model) {
        alertBox(t("@MSG_NO_MODEL_SELECTED"));
        return;
      }

      const checkReq = await api<any>("get", "modelapprove/approvecheck", {modelCode: model.modelCode});
  
      if(checkReq.data > 0){
        alertBox(t('@NG_APPROVAL_ERR'));
        operEqpSearchHandler(model.modelCode);
        return;
      }
  
      const data: Dictionary = {
        modelCode: model.modelCode,
      };
  
      const result = await api<any>("put", "modelrecipeparammapnew/cancellation", data);
      
      if (result.data > 0) {
        alertBox(t("@MSG_ALRAM_TYPE13"));
        operEqpSearchHandler(data.modelCode);
      }
    };
  
    const addHandler = async () => {
      
      const model = getSelectedModel();
      if (!model) {
        alertBox(t("@MSG_NO_MODEL_SELECTED"));
        return;
      }

      const checkReq = await api<any>("get", "modelapprove/approvecheck", {modelCode: model.modelCode});
  
      if(checkReq.data > 0){
        alertBox(t('@NG_APPROVAL_ERR'));
        operEqpSearchHandler(model.modelCode);
        return;
      }

      const check = await api<any>("get", "modelrecipeparammapnew/checkreg", {modelCode: model.modelCode});
  
      if(check.data > 0){
        alertBox("등록 절차가 완료되었습니다");
        operEqpSearchHandler(model.modelCode);
        return;
      }

      const operEqp = getSelectedOperEqp();
      if (!operEqp) {
        alertBox(t("@MSG_NO_EQUIPMENT_SELECTED"));
        return;
      }
  
      const groupCode = getSelectedGroup();
  
      if(groupCode && groupCode.checkbox != groupCode.recipeChangeYn)
      {
          alertBox("업데이트 중 오류가 발생했습니다");
          return;
      }
  
      const data: Dictionary = {
        modelCode: model.modelCode,
        operationSeqNo: operEqp.operationSeqNo,
        operationCode: operEqp.operationCode,
        eqpCode: operEqp.equipmentCode,
        interlockYn: operEqp.interlockYn,
        recipeChangeYn: !groupCode ? 'N' : groupCode.recipeChangeYn,
        groupCode: !groupCode ? null : groupCode.groupCode,
      };
  
      const result = await api<any>("post", "modelrecipeparammapnew/modelupdate", data);
      
      if (result.data > 0) {
        alertBox(t("@MSG_ALRAM_TYPE13"));
        selectRef.current.value = 'S';
        //searchHandler();
        //modelRef.current!.api.selectAll();
        operEqpSearchHandler(data.modelCode);
        //const cancel = await api<any>("put", "modelrecipeparammapnew/cancellation", {modelCode: model.modelCode});
      }
      else 
      {
        alertBox("등록 절차가 완료되었습니다");
        return;
      }
    };
  
  
    const editCompleteHandler = async (row: Dictionary, initRow: Dictionary) => {
      const newRow = {...initRow, ...row};

      let modelCode  =   newRow.modelCode;
      let modelName  =   newRow.modelName;
      let type  =   newRow.type;
      let revNote  =   newRow.revNote;

      if(initRow.modelCode){
        let formData = new FormData();
        formData.append('files', newRow.files);
        const result = await api<any>("post", "modelapprove/registration?modelCode="+modelCode+"&modelName="+modelName+"&type="+type+"&revNote="+revNote+"", formData);
  
        if(result.data > 0){
          selectRef.current.value = 'N';
          searchHandler();
          closeModal();
  
          addRef.current.style.backgroundColor = "#51d28c";
          addRef.current.style.borderColor = "#51d28c";
          addRef.current.disabled = true;
          const btnReg = document.getElementById('buttonRegistration');
          if (btnReg && btnReg !== undefined) { 
            btnReg.innerText = ' 승인요청중 ';
          }
        }
      }
      else
      {
        closeModal();
        alertBox(t("@MSG_NO_MODEL_SELECTED"));
      }
    };
  
    const editHandler = async () => {
  
      const model = getSelectedModel();
      if (!model) {
        alertBox(t("@MSG_NO_MODEL_SELECTED"));
        return;
      }
  
      const result = await api<any>("get", "modelrecipeparammapnew/check", {modelCode: model.modelCode});
  
      if(result.data > 0){
        setForm({ modelCode: model.modelCode, modelName: model.modelDescription });
      }
      else {
        alertBox("등록 절차가 완료되지 않았습니다");
        return;
      }
    }
  
    return (
      <>
        <ListBase
          folder="Reference Management"
          title="Recipe Model Mapping"
          icon="box"
          buttons={[]}
          search={
            <SearchBase ref={searchRef} searchHandler={searchHandler}
              preButtons={
                <>
                  <Button innerRef={addRef} type="button" color="danger" onClick={editHandler}>
                    <i className="uil uil-pen me-2"></i><span id="buttonRegistration">Recipe 승인 요청</span>
                  </Button>       
                </>
              }
              style={{ maxWidth: '400px !important' }}
            >
              <Row>
                <Col>
                  <Select
                    name="itemCategoryCode"
                    style={{ maxWidth: "140px" }}
                    placeholder={`${t("@PRODUCT")}/${t("@SEMI_FINISHED_PRODUCT")}`}
                    mapCode="code"
                    category="MODEL_TYPE"
                    className="form-select"
                    defaultValue={"FG"}
                    required={true}
                  />
                </Col>
                <Col>
                  <AutoCombo
                    name="modelCode"
                    sx={{ minWidth: "270px" }}
                    placeholder={t("@COL_MODEL_CODE")}
                    mapCode="model"
                  />
                </Col>
                <Col>
                  <Input
                    name="modelDescription"
                    placeholder={t("@COL_MODEL_NAME")}
                    style={{ minWidth: "250px" }}
                  />
                </Col>
                <Col>
                  <select
                    name="approveYn"
                    className="form-select"
                    style={{ minWidth: 130 }}
                    ref={selectRef}
                  >
                    <option value="">{t("@APPROVAL_STATUS")}</option>
                    <option value="S">{t("@EDITING")}</option>
                    <option value="N">{t("@APPROVAL_REQUEST")}</option>
                    <option value="Y">{t("@APPROVAL_COMPLETED")}</option>
                    <option value="R">{t("@REJECT")}</option>
                    {/* <option value="D">{t("@RE_APPROVAL_REQUEST")}</option> */}
                  </select>
                </Col>
              </Row>
            </SearchBase>
          }
        >
          <Row style={{ height: "88%" }}>
            <Col md={4}>
              <div className="pb-2" style={{ height: "100%" }}>
                <GridBase
                  ref={modelRef}
                  columnDefs={columnModelListDefs()}
                  onRowSelected={modelSelectedHandler}
                  rowMultiSelectWithClick={false}
                />
              </div>
            </Col>
            <Col md={8}>
              <div className="pb-2" style={{ height: "100%" }}>
                <GridBase
                  ref={eqpRef}
                  columnDefs={columnOperEqpDefs()}
                  onRowSelected={operEqpSelectedHandler}
                  suppressRowClickSelection={true}
                  onCellClicked={(event: CellClickedEvent) => {
                    if (
                      event.colDef.field == "interlockYn" ||
                      event.colDef.field == "rowspan"
                    ) {
                      return;
                    }
  
                    if (event.data.eqpYn != "Y") return;
  
                    event.node.setSelected(true);
                  }}
                  rowClassRules={{
                    "row-disable-container": (param: RowClassParams) => {
                      return param.data.eqpYn == "N" || !param.data.eqpYn;
                    },
                  }}
                />
              </div>
            </Col>
            <Col md={4}>
              <div className="pb-2" style={{ height: "100%" }}>
                <GridBase
                  ref={groupRef}
                  columnDefs={columnRecipeDefs()}
                  onRowSelected={recipeRowSelectedHandler}
                />
              </div>
            </Col>
            <Col md={3}>
              <div className="pb-2" style={{ height: "100%" }}>
                <GridBase ref={categoryRef} columnDefs={columnCategoryDefs()} />
              </div>
            </Col>
            <Col md={5}>
              <div className="pb-2" style={{ height: "100%" }}>
                <GridBase ref={paramRef} columnDefs={columnParamDefs()} />
              </div>
            </Col>
          </Row>
          <Row style={{ height: "12%" }}>
            <Col md={6} >
              <div className="pb-2" style={{ height: "100%" }}>
                <GridBase
                      ref={registedRef}
                      columnDefs={columnDetailDefs}
                      className="ag-grid-bbt"
                      containerId="grid-bbt-wrap"
                      alwaysShowHorizontalScroll={false}
                      onGridReady={() => {
                        setRegistedList([]);
                      }}
                  />
              </div>
            </Col>
            <Col md={6}>
            <br/><br/>
            <div className="d-flex gap-2 justify-content-end">
              {
              <Button type="button" color="warning" onClick={addHandler}>
                <i className="uil uil-pen me-2"></i> 항목별 저장
              </Button>
              }
              {
              <Button type="button" color="primary" onClick={regisHandler}>
                <i className="uil uil-pen me-2"></i> 공정별 확정
              </Button>
              }
              {
              <Button type="button" color="danger" onClick={cancelHandler}>
                <i className="uil uil-trash me-2"></i> 취소
              </Button>
              }
            </div>
            </Col>
          </Row>
        </ListBase>
        <ModelMapEdit ref={editRef} onComplete={editCompleteHandler} />
      </>
    );
  };
  
  export default ModelMapNewList;