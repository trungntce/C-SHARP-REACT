...
import { DefectRollDefs } from "./DefectRollDefs";
import DefectRollEdit from "./DefectRollEdit";
import DefectRollCancelEdit from "./DefectRollCancelEdit";

// 1. App.tsx 라우터 설정
// 2. Menu.tsx에서 사이드메뉴 추가

const DefectRollList = (props:any) => {
  const [searchRef, getSearch] = useSearchRef();
  const [gridRef, setList] = useGridRef();
  const [editRef, setForm, closeModal] = useEditRef();
  const [editCancelRef, setCancelForm, closeCancelModal] = useEditRef();

  const { refetch, post, put, del } = useApi("defectroll", getSearch, gridRef);
  ...
  const editCancelHandler = async (row: Dictionary, initRow: Dictionary) => {
    ...
    if(initRow.rollId){
      ...
      api<any>("put", "defectroll/off", newRow).then((result) => {
        if(result.data > 0){
          searchHandler();
          closeCancelModal();
        ...
        leftButtons={
          <div className="d-flex gap-2 justify-content-start">
            ...
            <Button type="button" color="primary" onClick={() => {
              ...
              <i className="uil uil-pen me-2"></i> 결함 등록
            </Button>
            <Button type="button" color="light" onClick={()=>{
              setCancelForm({})
            }}>
              <i className="uil uil-trash me-2"></i> 결함 해제
            </Button>
            {props.postButtons }
          </div>
        }
