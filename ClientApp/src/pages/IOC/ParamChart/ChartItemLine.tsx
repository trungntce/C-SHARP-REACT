...
const ParamChartItemLine = forwardRef((props: any, ref: any) => {  
  ...
  const options: any = {
    ...
    colors: ['#0099ec', '#fffa1e', '#ff373d', '#45ff24'],
  }
  ...
  const drawChart = () => {
    ...
    executeIdle(async () => {
      ...
      for(let i = 0; i < cols.length; i++){
        ...
        const result = await api<Dictionary[]>("get", "trace/rawcandlemin", param);
        const list = result.data;

        if(!list?.length)
          continue;

        const serie = list.map(x => { return { y: [x.startVal], x: new Date(x.inserttime) } });
        ...
        const avg = list.reduce((a, b) => a + b.startVal, 0) / list.length;
        const last = list[list.length - 1].startVal;

        options.yaxis.min = Math.min(options.yaxis.min ?? min, min);
        options.yaxis.max = Math.max(options.yaxis.max ?? max, max);

        minRef.current.innerHTML += (i > 0 ? " / " : "") + `<span>${options.yaxis.min?.toFixed(3)}</span>`;
        maxRef.current.innerHTML += (i > 0 ? " / " : "") + `<span>${options.yaxis.max?.toFixed(3)}</span>`;
        avgRef.current.innerHTML += (i > 0 ? " / " : "") + `<span>${avg?.toFixed(3)}</span>`;
        lastRef.current.innerHTML += (i > 0 ? " / " : "") + `<span>${last?.toFixed(3)}</span>`;
        
        seriesList.push({ name: `${cols[i].label}`, type: "line", data: serie });
      }

      if(seriesList.length <= 0)
        return;

      ApexCharts.exec("rawchart-" + props.index, "updateOptions", { ...options });
...
export default ParamChartItemLine;