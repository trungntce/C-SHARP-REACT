...
(
	...
	where
		equip not in ('M-102-01-V001','M-116-01-V002','M-117-01-V002','M-102-01-V001','M-102-01-V002','M-117-01-V006','M-116-01-V002')
		and main = 'y'
	union
	select distinct
		eqcode		as eqp_code
...
begin
	if @table_name = 'raw_aoi_orbotech'
		begin
			...
			select
				*
			,	datediff(ss, isnull(lag(dasinserttime) over (order by dasinserttime asc),@from_dt ), dasinserttime) as diff
			,	datediff(ss, isnull(lag(dasinserttime) over (order by dasinserttime asc),@from_dt ), dasinserttime) - 5 * 60 as diff_over5min
			from
				cte
		...
		begin	
			...
			(
				...
				where
					equip = @eqp_code and dasinserttime between @from_dt and @to_dt
					and message in ('message: table_move_to(): index =0, x=0, y=24000','message: table_move_to(): index =0, x=100, y=23000')
			),cte_diff as
			(
			select
				*
	...
	select
		case when sum_diff_over5min is null then datediff(ss,@from_dt,@to_dt) else sum_diff_over5min end AS non_time
