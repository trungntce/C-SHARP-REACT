...
import { favoriteList, favoriteListToMenuItemList, getFavorite, globalContext, menuItemsMainPage, menuItemsDb, menuItemsFavorite, MenuItemsProps, refreshFavorite } from "./Menu";
    ...
    return (
        <div >
            <div className=' widthtop'>
                ...
                <li className="menu-title itext" data-key="t-layouts" id={item.menuId}>{item.label}</li>
            </div>         
...
export const MenuItem = ({ item, props }: any) => {
  ...
  return (
      <li className={item["className"]} data-menuid={item["menuId"]} id={item["menuId"]}>
          <div className={(hasChildren || item["menuType"] == 'M' ? "midparent" : "") }>
      <Link to={url} target={item["target"]} data-menuid={item["menuId"]} className={classNames(
      ...
      )}>
        {
          item["icon"] && <Icon name={item["icon"]} className="icon nav-icon" />
                  ...
                  <span className="menu-item">{(item["menuType"] != 'M' && !hasChildren?(1+item["childid"])+". ":"") + props.t(item["label"])}</span>
        {item.badge &&
...
const SidebarContent = (props: any) => {
  ...
  return (
      <React.Fragment key={"otherkeydat"}>      
     
        {/*<div>*/}
        {/*  <AutoCombo name="vendorCode" placeholder="메뉴검색" mapCode="menusearch" */}
        {/*    ref={menuSearchRef}*/}
        {/*    disablePortal={false} */}
        {/*    hideValue={true} */}
        {/*    onChange={(event:any, value: Dictionary) => {*/}
        {/*      menuSearchRef.current.setValue({ value: "", label: "" });*/}
        {/*      const path = value.parent;*/}
        {/*      path && navigate(path);*/}
        {/*    }}*/}
        {/*    />*/}
        {/*</div>*/}
          ...
          {
              (menuItemsMainPage || []).map((item, key) => {
                  //  console.log(key,item);
                  //if (key == 0) console.log(menuItemsAll);
                      ...
                      return (
                          <React.Fragment key={key}>
                              ...
                              <Col md={item.label == 'undeveloped' ? 4 : 2} >
                                      <ul className="metismenu list-unstyled2" >
                                  ...
                                  <div className={"width16"}>                                            
                                      ...
                                      <HeaderMenuItem item={item} />
                                              {
                                                (item.subItems || []).map((ichild:any, kchild:any) => {

                                                     //console.log(ichild);
                                                   
                                                    return <MenuItem item={ichild} props={props} key={ ichild.menuId} />
          ...
          <SimpleBar ref={ref} className="sidebar-menu-scroll" id="nav-scroll">     
              <div id="sidebar-menu2">
                  <ul className="metismenu list-unstyled2" id="side-menu2"></ul>
        </div>
      </SimpleBar>
    </React.Fragment>
    );
