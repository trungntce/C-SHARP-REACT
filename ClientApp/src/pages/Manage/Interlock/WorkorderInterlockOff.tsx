...
const WorkorderInterlockOff = forwardRef((props: any, ref: any) => {
  ...
  const submitHandler = async (formData: FormData, row: Dictionary) => {
    ...
    confirmBox(t("@MSG_ASK_INTERLOCK_RELEASE"), async () => { //인터락 해제 하시겠습니까?

      const { hideProgress, startFakeProgress } = showProgress("Transaction progress", "progress");
      startFakeProgress();
      const result = await api<any>("post", "panelinterlock/workorder", param);
      hideProgress();  
      
      if (result.data == -1) {
        alertBox(`Batch: ${param.workorder} ${t("@MSG_NO_SETUP_INTERLOCK")}`);  //Batch: ${param.workorder} 는 설정된 인터락이 없습니다.
        return;
      }else if(result.data < 0){
        alertBox(t("@MSG_ERROR_RELEASE_")); //해제 중 오류가 발생했습니다.
        return;
      }
  ...
  useImperativeHandle(ref, () => ({ 
    setList: (rows: Dictionary[]) => {
      ...
      if(find){
        alertBox(`Batch: ${find.workorder}, ${t("@OPER_SEQ")}: ${find.operSeqNo} ${t("@MSG_NO_SETUP_INTERLOCK_ITEM")}`);  //Batch: ${find.workorder}, 공순: ${find.operSeqNo} 항목은 인터락이 설정되어있지 않습니다.
        return false;
      }
        ...
        <Col md={7}>
                ...
                <ButtonGroup>
                  ...
                  <Button type="button" color="primary" onClick={() => {
                    ...
                    if (isNullOrWhitespace(workorder)) {
                      alertBox(`Batch${t("@MSG_PLEASE_ENTER_NUMBER")}`);  //Batch번호를 입력해 주세요.
                      return;
                    }
          ...
          <Row>
            <Col md={5}>
              <div className="mb-3">
                <Label className="form-label" htmlFor="judgeCode">
                  {t("@PANEL_INTERLOCK_FUCTIONRELEASE_USE")} {/*  판넬 인터락 해제 여부*/}
                </Label>
          ...
          <Row>
            <Col>
              <div className="mb-3">
                ...
                <Input name="remark" type="text" placeholder={t("@RELEASE_REASON")} autoComplete="off" />
              </div>
            </Col>
          </Row>
        </Col>
...
export default WorkorderInterlockOff;
