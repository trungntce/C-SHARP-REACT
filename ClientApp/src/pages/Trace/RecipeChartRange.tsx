...
const RecipeChartRange = forwardRef((props: any, ref: any) => {
  ...
  const setChart = async (row: Dictionary) => {
    ...
    if (result.data?.length) {
      ...
      ApexCharts.exec("recipeChart", "updateSeries", 
        [
          { type: "rangeArea", data: serieMinMax, name: `[${row.columnName}] ${row.recipeName} 최소~최대` }, 
          { type: "line", data: serie, name: `[${row.columnName}] ${row.recipeName} 평균` }
        ]);
    } else {
...
export default RecipeChartRange;
