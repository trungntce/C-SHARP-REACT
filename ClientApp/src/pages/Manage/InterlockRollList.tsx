...
import { InterlockRollDefs } from "./InterlockRollDefs";
import InterlockRollEdit from "./InterlockRollEdit";
import InterlockRollCancelEdit from "./InterlockRollCancelEdit";

// 1. App.tsx 라우터 설정
// 2. Menu.tsx에서 사이드메뉴 추가

const InterlockRollList = (props:any) => {
  const [searchRef, getSearch] = useSearchRef();
  const [gridRef, setList] = useGridRef();
  const [editRef, setForm, closeModal] = useEditRef();
  const [editCancelRef, setCancelForm, closeCancelModal] = useEditRef();

  const { refetch, post, put, del } = useApi("interlockroll", getSearch, gridRef);
    ...
    if(initRow.rollId){
      ...
      api<any>("put", "interlockroll/set", newRow).then((result) => {
        ...
        if(result.data > 0){
          ...
          alertBox(`동일한 항목이 존재합니다.<br />Worker Id: ${newRow.rollId}`);
  ...
  const editCancelHandler = async (row: Dictionary, initRow: Dictionary) => {
    ...
    if(initRow.rollId){
      ...
      api<any>("put", "interlockroll/off", newRow).then((result) => {
        if(result.data > 0){
          searchHandler();
          closeCancelModal();
          alertBox("해제가 완료되었습니다.");
        }else if(result.data == -1){
        ...
        leftButtons={
          <div className="d-flex gap-2 justify-content-start">
            ...
            <Button type="button" color="primary" onClick={() => {
              ...
              <i className="uil uil-pen me-2"></i> 인터락 등록
            </Button>
            <Button type="button" color="light" onClick={()=>{
              setCancelForm({})
            }}>
              <i className="uil uil-trash me-2"></i> 인터락 해제
            </Button>
            {props.postButtons }
          </div>
        }
...
export default InterlockRollList;
