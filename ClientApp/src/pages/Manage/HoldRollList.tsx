...
import { HoldRollDefs } from "./HoldRollDefs";
import HoldRollEdit from "./HoldRollEdit";
import HoldRollCancelEdit from "./HoldRollCancelEdit";

// 1. App.tsx 라우터 설정
// 2. Menu.tsx에서 사이드메뉴 추가

const HoldRollList = (props:any) => {
  const [searchRef, getSearch] = useSearchRef();
  const [gridRef, setList] = useGridRef();
  const [editRef, setForm, closeModal] = useEditRef();
  const [editCancelRef, setCancelForm, closeCancelModal] = useEditRef();

  const { refetch, post, put, del } = useApi("holdroll", getSearch, gridRef);
  ...
  const editCompleteHandler = async (row: Dictionary, initRow: Dictionary) => {
    ...
    if(initRow.rollId){
      ...
      api<any>("put", "holdroll/set", newRow).then((result) => {
        if(result.data > 0){
          searchHandler();
          closeModal();
  ...
  const editCancelHandler = async (row: Dictionary, initRow: Dictionary) => {
    ...
    if(initRow.rollId){
      ...
      api<any>("put", "holdroll/off", newRow).then((result) => {
        if(result.data > 0){
          searchHandler();
          closeCancelModal();
