...
import { EquipForM, EquipStatus } from "../../StateStore/EquipMentList";
...
const EquipList = ({ selectFnc }: Props) => {
  ...
  // useEffect(() => {
  //   const tttt = async () => {
  //     const { data } = await api<any>("get", "MonitoringDetail/eqp4mmodel", {});
  //     if (data) {
  //       setModels(data);
  //     }
  //   };
  ...
  useEffect(() => {
    const fillter = [...data].filter(
      (f: any) => f.facNo === facno && f.eqpType === eqptype
    );
    setGetData(fillter);

    const oee_sum = fillter?.reduce(
      (pre: any, cur: any) => pre + cur.timeRate,
      0
    );
    const oee_avg = oee_sum / fillter.length;
    ...
    setTotal(
      fillter?.reduce(
        (pre: any, cur: any) => {
          return {
            ...pre,
            planCnt: pre.planCnt + cur.planCnt,
            prodCnt: pre.prodCnt + cur.prodCnt,
            expectedProd: pre.expectedProd + cur.expectedProd,
          };
        },
        {
          planCnt: 0,
    ...
    const filtered = fillter?.find((i: any) => i.eqpCode === eqp);
                ...
                <div className={listStyle.listBoxContent}>
                  <div style={{ padding: "3px 10px" }}>
                    ...
                    >
                      ...
                      {Cell(toComma(Math.round(i?.timeRate ? i?.timeRate : 0)),"#F28705")}
                    </div>
                  </div>
                </div>
              </div>
